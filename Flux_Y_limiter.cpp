		...
		for ( j = 1; j <= ny; j++) {
			for (k = 2; k <= nz; k++) {
				...
				if (j <= 2 | j >= ny-1) {
					...
					//jL = 1./(2/J[i][j-1][k]+5/J[i][j][k]-1/J[i][j+1][k]);
					//jR = 1./(-1/J[i][j-1][k]+5/J[i][j][k]+2/J[i][j+1][k]);
					...
					//rL = (u1i - u1)/(( u1  - iu1))*J_v[i-1][j][k-1]/J_v[i-1][j-1][k-1];
					//rR = (u1 - iu1)/(( u1i - u1 ))*J_v[i-1][j-1][k-1]/J_v[i-1][j][k-1];
					
					rL = (u1i - u1)/(( u1  - iu1)+0.0000001);
					rR = (u1 - iu1)/(( u1i - u1 )+0.0000001);
					...
					//ML1[i-1][j][k-1] = (u1+0.5*max(0,minL3)*(u1  -iu1))*jL;
					//MR1[i-1][j-1][k-1] = (u1-0.5*max(0,minR3)*(u1i - u1))*jR;
					
					ML1[i-1][j][k-1] = (u1*J[i][j][k]+0.5*max(0,minL3)*(u1  -iu1));
					
					MR1[i-1][j-1][k-1] = (u1*J[i][j][k]-0.5*max(0,minR3)*(u1i - u1));

					
					if (myid == 0 && i == 3 && k == 20) printf("%f\t%f\t%f\t%f\t%d\n",ML1[i-1][j][k-1],MR1[i-1][j-1][k-1],minR3 ,minL3,j);
					...
					ML2[i-1][j  ][k-1] = (u2+0.5*max(0,minL3)*(u2  -iu2))*jL;
					...
					ML3[i-1][j  ][k-1] = (u3+0.5*max(0,minL3)*(u3  -iu3))*jL;
					...
					ML4[i-1][j  ][k-1] = (u4+0.5*max(0,minL3)*(u4  -iu4))*jL;
					...
					ML5[i-1][j  ][k-1] = (u5+0.5*max(0,minL3)*(u5  -iu5))*jL;
	...
	for (i = istart; i <= iend; i++) {
		for (j = 1; j < nyy; j++) {
			for (k = 1; k < nz; k++) {
				...
				inFy1[i][j][k] = 0.5*((_rho*_V+rho_*V_)-EpY[i][j][k]*Fav1)/J_v[i][j][k];
				inFy2[i][j][k] = 0.5*((_rho*_u*_V+rho_*u_*V_+etx*(_P+P_))-EpY[i][j][k]*Fav2)/J_v[i][j][k];
				inFy3[i][j][k] = 0.5*((_rho*_v*_V+rho_*v_*V_+ety*(_P+P_))-EpY[i][j][k]*Fav3)/J_v[i][j][k];
				inFy4[i][j][k] = 0.5*((_rho*_w*_V+rho_*w_*V_+etz*(_P+P_))-EpY[i][j][k]*Fav4)/J_v[i][j][k];
				inFy5[i][j][k] = 0.5*((_V*(3.5*_P+0.5*_rho*_VV)+V_*(3.5*P_+0.5*rho_*VV_))-EpY[i][j][k]*Fav5)/J_v[i][j][k];
				
				if (j==1 | j==ny) {

					inFy1[i][j][k] = 0;
					inFy2[i][j][k] = 0;
					inFy3[i][j][k] = 0.5*(ety*(_P+P_)-EpY[i][j][k]*Fav3)/J_v[i][j][k];
					inFy4[i][j][k] = 0;
					inFy5[i][j][k] = 0;
